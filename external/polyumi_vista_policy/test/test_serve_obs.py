...
#
# These must reproduce VistaDataset.__getitem__ for the same keys, because the policy compares
# only like with like: the ROS node reproduces what the EXPORTER stores, and these functions
# reproduce what the DATASET does when it loads that. A mismatch here is invisible at runtime —
# a valid tensor of the right shape carrying the wrong numbers.

AUDIO_ROWS = 10
MIC0_WIDTH = 536
FINGER_H, FINGER_W = 648, 982  # the exporter's native crop of the 1152x648 finger view


def _finger_stack(rng, n=TO):
    """Finger frames as the wire carries them: uint8, at the exporter's crop size."""
    return rng.integers(0, 256, size=(n, FINGER_H, FINGER_W, 3), dtype=np.uint8)


def test_tactile_keys_absent_when_not_supplied():
    # A visuomotor policy must not be handed keys it has no encoder for.
    rng = np.random.default_rng(0)
    obs = wire_to_obs_dict(
        rng.integers(0, 256, size=(TO, H, W, 3), dtype=np.uint8),
        np.stack([_agent_pos([0, 0, 0], [0, 0, 0])] * TO),
    )
    assert 'finger_rgb' not in obs
    assert 'mic_0' not in obs


def test_finger_rgb_shape_and_range():
    rng = np.random.default_rng(1)
    obs = wire_to_obs_dict(
        rng.integers(0, 256, size=(TO, H, W, 3), dtype=np.uint8),
        np.stack([_agent_pos([0, 0, 0], [0, 0, 0])] * TO),
        finger_rgb=_finger_stack(rng),
    )
    # Batched, channel-first, square at the encoder's input size, scaled to [0, 1].
    assert obs['finger_rgb'].shape == (1, TO, 3, 224, 224)
    assert obs['finger_rgb'].dtype == np.float32
    assert 0.0 <= obs['finger_rgb'].min() and obs['finger_rgb'].max() <= 1.0


def test_finger_rgb_matches_the_dataset_transform_exactly():
    # The dataset casts to float32 BEFORE resizing, so cv2 runs its float kernel. Resizing the
    # uint8 first would round differently — a small, silent train/inference skew.
    from vista.data.vista_dataset import FINGER_RGB_SIZE, _center_crop_square_resize

    rng = np.random.default_rng(2)
    finger = _finger_stack(rng)
    obs = wire_to_obs_dict(
        rng.integers(0, 256, size=(TO, H, W, 3), dtype=np.uint8),
        np.stack([_agent_pos([0, 0, 0], [0, 0, 0])] * TO),
        finger_rgb=finger,
    ...
    expected = finger.astype(np.float32)
    expected = np.stack([_center_crop_square_resize(f, FINGER_RGB_SIZE) for f in expected])
    expected = np.moveaxis(expected, -1, 1) / 255.0

    np.testing.assert_array_equal(obs['finger_rgb'][0], expected.astype(np.float32))


def test_mic0_passes_through_unscaled():
    # mic_0 is a raw waveform the exporter already normalised to [-1, 1); the log-mel lives in
    # the model. Scaling it here (as the RGB path does) would silently attenuate it.
    rng = np.random.default_rng(3)
    mic = rng.uniform(-1.0, 1.0, size=(AUDIO_ROWS, MIC0_WIDTH)).astype(np.float32)
    obs = wire_to_obs_dict(
        rng.integers(0, 256, size=(TO, H, W, 3), dtype=np.uint8),
        np.stack([_agent_pos([0, 0, 0], [0, 0, 0])] * TO),
        mic_0=mic,
    )
    assert obs['mic_0'].shape == (1, AUDIO_ROWS, MIC0_WIDTH)
    np.testing.assert_array_equal(obs['mic_0'][0], mic)


def test_audio_horizon_is_independent_of_the_image_horizon():
    # mic_0's leading dimension is the audio horizon (10 contiguous rows), not To (2). The frame
    # format carries per-channel shapes precisely so these need not agree.
    rng = np.random.default_rng(4)
    obs = wire_to_obs_dict(
        rng.integers(0, 256, size=(TO, H, W, 3), dtype=np.uint8),
        np.stack([_agent_pos([0, 0, 0], [0, 0, 0])] * TO),
        finger_rgb=_finger_stack(rng),
        mic_0=rng.uniform(-1, 1, size=(AUDIO_ROWS, MIC0_WIDTH)).astype(np.float32),
    )
    assert obs['camera0_rgb'].shape[1] == TO
    assert obs['finger_rgb'].shape[1] == TO
    assert obs['mic_0'].shape[1] == AUDIO_ROWS != TO
