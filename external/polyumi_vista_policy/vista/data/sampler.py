# Derived from diffusion_policy/common/sampler.py (UMI diffusion_policy fork).
# Added: audio_keys branch with zero-padding at episode start (ManiWAV convention).
    ...
    def __init__(
        ...
        for i in range(len(episode_ends)):
            ...
            for current_idx in range(start_idx, end_idx):
                if not action_padding and end_idx < current_idx + (
                    key_horizon["action"] - 1
    ...
    def sample_sequence(self, idx):
        ...
        for key in obs_keys:
            ...
            if key in self.rgb_keys:
                ...
                num_valid = min(
                    this_horizon, (current_idx - start_idx) // this_downsample_steps + 1
                )
                ...
                # Contiguous when down_sample_steps==1 (SHF/Sparsh audio_obs_horizon);
                # otherwise same strided grid as RGB. Zero-pad missing history.
                num_valid = min(
                    this_horizon,
                    (current_idx - start_idx) // this_downsample_steps + 1,
                )
                slice_start = current_idx - (num_valid - 1) * this_downsample_steps
                output = input_arr[slice_start : current_idx + 1 : this_downsample_steps]
                assert output.shape[0] == num_valid
                if output.shape[0] < this_horizon:
                    pad = np.zeros(
                        (this_horizon - output.shape[0], *output.shape[1:]),
                        dtype=output.dtype,
                    )
                    output = np.concatenate([pad, output], axis=0)

            else:
                idx_with_latency = np.array(
                    [
                        current_idx - i * this_downsample_steps + this_latency_steps
                        for i in range(this_horizon)
                    ],
                    dtype=np.float32,
                )
                ...
                if "rot" in key:
                    ...
                    slerp = st.Slerp(
                        ...
                        rotations=rot_preprocess(
                            input_arr[interpolation_start:interpolation_end]
                        ),
                    )
