# polyumi_vista_policy

Private PolyUMI / Vista research fork of the UMI diffusion-policy codebase.

**Multimodal Vista policies:** see [`docs/VISTA_AGENT_GUIDE.md`](docs/VISTA_AGENT_GUIDE.md) for architecture zoo, training, and agent handoff.

Baseline: [anon-authors/polyumi_diffusion_policy](https://github.com/anon-authors/polyumi_diffusion_policy).

- **origin** — your private `polyumi_vista_policy` repo (model experiments land here)
- **upstream** — public PolyUMI diffusion-policy fork (pull platform fixes from here)

PolyUMI wires training via `.vista_policy.env` → `train_policy.sh` / `serve_policy.sh`.
Run `./setup_vista_policy.sh` from the PolyUMI root after a fresh clone.
