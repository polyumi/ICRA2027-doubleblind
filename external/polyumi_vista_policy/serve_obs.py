...
def wire_to_obs_dict(
    ...
    Args:
        ...
        finger_rgb: ``[To, H, W, 3]`` finger-camera frames at the size the EXPORTER stores —
            cropped to the useful region, not resized. ``None`` for a visuomotor policy.
        mic_0: ``[Ta, 536]`` contact-mic rows, float32 already in ``[-1, 1)``. Note the leading
            dimension is the *audio* horizon, which is longer than ``To`` and downsampled
            differently; the frame format carries per-channel shapes for exactly this reason.
            ``None`` for a visuomotor policy.

    Returns:
        dict of ``np.float32`` arrays keyed by the exact ``shape_meta`` obs names. The two
        tactile keys are present only when their argument was supplied — a policy whose
        ``sensor_group`` omits a modality must not be handed it, and one that needs it must not
        silently receive nothing (the wire contract rejects that upstream, in
        ``Observation.require``).
    ...
    if finger_rgb is not None:
        # Reproduce VistaDataset.__getitem__ for this key, in ITS order: cast to float32, then
        # centre-crop-and-resize, then move axes and scale. Resizing before the cast would run
        # cv2's uint8 kernel instead of its float one and round differently — a small, invisible
        # train/inference skew of exactly the kind this translation layer exists to prevent.
        #
        # The dataset's own function is reused rather than reimplemented, so the two cannot drift.
        # Imported lazily because vista.data.vista_dataset pulls torch, zarr and the whole
        # diffusion_policy stack, and this module is deliberately importable (and unit-testable)
        # without a GPU or a checkpoint — see the module docstring.
        from vista.data.vista_dataset import FINGER_RGB_SIZE, _center_crop_square_resize

        finger = np.asarray(finger_rgb).astype(np.float32)
        finger = np.stack([_center_crop_square_resize(frame, FINGER_RGB_SIZE) for frame in finger])
        obs['finger_rgb'] = np.moveaxis(finger, -1, 1) / 255.0  # [To, 3, 224, 224]

    if mic_0 is not None:
        # Straight cast, no scaling: mic_0 is a raw waveform the exporter already normalised to
        # [-1, 1), and the log-mel is computed inside the model (after waveform-domain
        # augmentation at training time), not here. The dataset does exactly this too.
        obs['mic_0'] = np.asarray(mic_0, dtype=np.float32)

    # Add the batch dim and cast to the float32 the policy expects. mic_0's leading dimension is
    # the audio horizon rather than To, which this handles unchanged.
