...
class JointInfo:
    ...
    name: str
    command_interfaces: list[InterfaceInfo] = field(default_factory=list)
    state_interfaces: list[InterfaceInfo] = field(default_factory=list)
...
class HardwareComponentInfo:
    ...
    gpios: list[GpioInfo] = field(default_factory=list)
...
def parse_hardware_components(urdf_string: str) -> list[HardwareComponentInfo]:
    """
...
def get_components_by_name(
    components: list[HardwareComponentInfo], name: str
) -> list[HardwareComponentInfo]:
    """Filter components by exact name."""
    return [c for c in components if c.name == name]


def get_arm_components(
    components: list[HardwareComponentInfo],
) -> list[HardwareComponentInfo]:
...
def get_joint_command_interface_names(component: HardwareComponentInfo) -> set[str]:
...
def get_gpio_indices(gpios: list[GpioInfo]) -> list[int]:
    """Return sorted list of indices from a GPIO list."""
    return sorted(g.index for g in gpios)


def get_all_registered_joint_names(components: list[HardwareComponentInfo]) -> set[str]:
